...
class SitAnymalCEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: SitAnymalCFlatEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_rewards(self) -> torch.Tensor:
        pos_tracking_coarse = position_error_tanh(
            self._robot.data.root_com_pos_w[:, :2], self._target_position[:, :2], std=2.0
        )
        pos_tracking_fine = position_error_tanh(
            self._robot.data.root_com_pos_w[:, :2], self._target_position[:, :2], std=0.2
        )
